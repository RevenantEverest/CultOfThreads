...
import { type UpdateOptions, update } from '~/modules/categories/actions';
    ...
    return useMutation({
        ...
        onError: (err) => {
            if(err instanceof AxiosError && err.response) {
                console.error(
                    "Backend error response in update category hook: ", 
                    err.response.data
                );
            }
            else {
                console.error("Generic error in update category hook: ", err);
            }
        }
    });
};