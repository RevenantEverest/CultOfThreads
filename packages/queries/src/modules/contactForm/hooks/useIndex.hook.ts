...
import { type FetchAllOptions, fetchAll } from '~/modules/contactForm/actions';
...
export async function usePrefetchIndex(queryClient: QueryClient, options: FetchAllOptions) {
    await queryClient.prefetchInfiniteQuery({
        ...
        initialPageParam: 1,
        getNextPageParam: (lastPage: PaginatedResponse<ContactForm>) => {
            return pagination.getNextPageParam<ContactForm>(lastPage)
        }
    });
};
